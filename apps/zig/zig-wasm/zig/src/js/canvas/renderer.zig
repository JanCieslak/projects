...
pub const CanvasRenderer = struct {
    ...
    pub fn translate(self: Self, x: f64, y: f64) void {
        _ = self.context.call("translate", .{ x, y });
    }

    pub fn rotate(self: Self, angle: f64) void {
        _ = self.context.call("rotate", .{angle});
    }
    ...
    pub fn fillRect(self: Self, x: f64, y: f64, w: f64, h: f64, angle: ?f64) void {
        if (angle) |a| {
            _ = a;
            // TODO: Wrong pivot point - should be center or an option
            // _ = self.context.set("lineWidth", @bitCast(Value, h / 2.0));
            // _ = self.context.call("beginPath", .{});
            // _ = self.context.call("moveTo", .{ x, y });
            // _ = self.context.call("lineTo", .{ x + w * std.math.cos(a), y + w * std.math.sin(a) });
            // _ = self.context.call("stroke", .{});
        } else {
            _ = self.context.call("fillRect", .{ x, y, w, h });
        }
    }

    pub fn fillCircle(self: Self, x: f64, y: f64, radius: u32) void {
        _ = self.context.call("beginPath", .{});
